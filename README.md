...
用于集中管理日常使用的 Pi 扩展（Extensions）、技能包（Skills）、提示词模板（Prompts）以及网络与环境配置文件。
...
│   ├── antigravity/               # 🪐 Antigravity 插件与国内代理完整接入指南
│   └── hello-pi/                  # 🚀 示例扩展插件
...
1. **📖 功能与内容介绍**：清晰描述该插件解决什么问题、支持哪些模型/能力。
2. **🚀 安装与使用方法**：具体的安装命令（npm/本地加载）与使用步骤（Slash 命令/快捷键）。
...
- **[Antigravity 插件与代理配置指南](./extensions/antigravity/README.md)**：解决国内访问 Google Cloud Code Assist 报 `fetch failed` 的终极方案与脚本。
