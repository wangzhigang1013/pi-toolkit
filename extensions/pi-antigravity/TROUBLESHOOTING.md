...
### 坑 2：Pi Web 中完全看不到思考过程（Thinking 折叠块）
...
* **现象**：模型回答的正文被全部截断折叠进思考卡片里，正文区一片空白。
* **底层根因**：
  - 如果在文本流解析中用简单的正则表达式扫描 `<think>` / `<thought>` 标签，一旦模型输出的代码、Markdown 文本（例如解释思考机制时）包含了这些字符，解析器状态机就会误以为进入了思考块，将后续的所有正常正文误当作思考数据分发到 `thinking_delta`。
* **解决方案**：
  - 坚决**不要在普通文本流中做模糊的 XML 思考标签拦截**。
  - 严格依赖 Google Cloud Code Assist 官方协议的 `part.thought === true` 字段作为思考判据。`thought === true` 发送到 `thinking_delta`，其余所有字符一律进入 `text_delta`。
