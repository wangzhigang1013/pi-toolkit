  ...
  if (model.reasoning) {
    ...
    if (effort === "off") {
      ...
      generationConfig.thinkingConfig = {
        ...
        ...(runtimeModel.includes("3.7") || runtimeModel.includes("tiered")
          ? {
              thinkingLevel:
                effort === "medium"
                  ? "MEDIUM"
...
async function streamResponse(
  ...
  const appendText = (delta: string, signature?: string) => {
    ...
    if (signature) currentBlock.textSignature = signature;
    stream.push({
      type: "text_delta",
      contentIndex: blockIndex(),
      delta,
  ...
  void runWithDiagnostics(async () => {
    ...
    try {
      ...
      const effort =
        opts.reasoning === "off"
          ? "off"
          : opts.reasoning || (model.reasoning ? "high" : "off");
      const effectiveOpts: AntigravityStreamOptions = {
        ...opts,
      ...
      const isClaudeReasoning = model.id.startsWith("claude-") && model.reasoning && effort !== "off";
      ...
      for (let emptyAttempt = 0; emptyAttempt <= 2; emptyAttempt++) {
        ...
        for (let candIdx = 0; candIdx < runtimeCandidates.length; candIdx++) {
          ...
          const body = JSON.stringify(
            buildRequest(model, context, projectId, effectiveOpts, runtimeModel),
          );
